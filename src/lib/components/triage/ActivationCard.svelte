...
<div class="space-y-3">
	...
	></div>
	<h3
		class="text-sm font-bold tracking-wide text-muted-foreground/60 [font-variant-caps:small-caps]"
	>
		Recommended Trauma Triage Level
	</h3>
	<div class="rounded-lg border {c.border} {c.bg} p-4 space-y-3">
		...
		<div class="flex items-start justify-between gap-2">
			<div>
				<h2
					class="font-mono text-2xl font-extrabold tracking-wider {c.text}"
				>
					{level.toUpperCase()}
				</h2>
			...
			{#if matches.length > 0}
				<span class="text-xs text-muted-foreground mt-1.5">
					{matches.length}
					{matches.length === 1 ? "criterion" : "criteria"}
				</span>
			{/if}
		</div>
		...
		{#if expanded && (justification || agentReasoning)}
			<div
				...
				class="rounded-md bg-muted/50 p-3 text-sm text-muted-foreground whitespace-pre-wrap space-y-3"
			>
				{#if justification}
					<div>
						<p class="font-medium text-foreground/70 mb-1">
							Justification
						</p>
						<p>{justification}</p>
					</div>
