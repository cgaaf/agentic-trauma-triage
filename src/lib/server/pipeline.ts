...
import { mergeMatches, determineActivationLevel, buildJustification } from '$lib/server/engine/merge.js';
...
export async function* runPipeline(report: string): AsyncGenerator<SSEEvent> {
	...
	if (!isTraumaReport) {
		yield {
			type: 'error',
			message: "This doesn't appear to be a trauma/EMS report. Please provide a trauma-related EMS narrative.",
			phase: 'extraction',
			canRetry: false,
		};
		return;
	...
	if (fields.age === null) {
		yield {
			type: 'error',
			message: 'Age could not be determined from the report. Age is required for triage evaluation.',
			phase: 'extraction',
			canRetry: false,
		};
		return;
	...
	if (fields.sbp === null) missingFieldWarnings.push('Without SBP, blood pressure criteria cannot be fully evaluated.');
	if (fields.hr === null) missingFieldWarnings.push('Without HR, heart rate criteria cannot be fully evaluated.');
	if (fields.rr === null) missingFieldWarnings.push('Without RR, respiratory rate criteria cannot be fully evaluated.');
	if (fields.gcs === null) missingFieldWarnings.push('Without GCS, Glasgow Coma Scale criteria cannot be fully evaluated.');
