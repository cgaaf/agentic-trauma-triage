...
describe('Criteria data integrity', () => {
	...
	it('every criterion passes Zod schema validation', () => {
		for (const criterion of CRITERIA) {
			...
			expect(result.success, `Criterion ${criterion.id} failed validation: ${JSON.stringify(result)}`).toBe(true);
		}
	});
	...
	it('every hybrid criterion has a vitalRule with requiresLlmConfirmation', () => {
		...
		for (const c of hybrid) {
			...
			expect(c.vitalRule!.requiresLlmConfirmation, `Criterion ${c.id} (hybrid) missing requiresLlmConfirmation`).toBeDefined();
		}
	});
