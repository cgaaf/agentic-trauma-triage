...
export async function mockExtract(report: string): Promise<{ fields: ExtractedFields; isTraumaReport: boolean }> {
	await delay(500);
	...
	const traumaKeywords = [
		'trauma', 'injury', 'accident', 'crash', 'fall', 'mvc', 'mcc', 'gsw',
		'gcs', 'sbp', 'hr', 'rr', 'blood pressure', 'heart rate', 'intubat',
		'fracture', 'wound', 'bleed', 'burn', 'penetrating', 'ejection',
		'ems', 'ambulance', 'paramedic', 'patient', 'yo', 'y/o', 'year old',
	];
	...
	const rr = extractNumber([
		/rr\s*:?\s*(\d+)/i,
		/resp(?:iratory)?\s*rate\s*:?\s*(\d+)/i,
	]);
	...
	const mechanisms = ['mvc', 'mcc', 'fall', 'crash', 'gsw', 'stabbing', 'pedestrian', 'ejection', 'bicycle'];
