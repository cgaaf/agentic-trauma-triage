...
export const EXTRACTION_TOOL: Anthropic.Messages.Tool = {
	...
	input_schema: {
		...
		required: ['isTraumaReport', 'age', 'sbp', 'hr', 'rr', 'gcs', 'airwayStatus', 'breathingStatus', 'mechanism', 'injuries', 'additionalContext'],
	},
};
...
export function buildEvaluationTool(hybridIds: number[]): Anthropic.Messages.Tool {
	return {
		...
		description: 'Evaluate trauma triage criteria against extracted patient data and return matches.',
		input_schema: {
			type: 'object' as const,
			properties: {
				matches: {
